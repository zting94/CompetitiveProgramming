...
const int mxN=1e4+1, mxM=5e4+1, mxK=21;
int N, M, K;
vii adj[mxN];

void solve() {
	cin >> N >> M >> K;
	vvl dist(N, vl(K+1, 1e18));
	for(int i=0; i<M; ++i) {
		int u, v, d;
		cin >> u >> v >> d, --u, --v;
		adj[u].push_back({v, d});
	...
	while(!pq.empty()) {
		...
		if(e.d>dist[e.u][e.k])
			continue;
		for(auto& ne : adj[e.u]) {
			if(e.d+ne.second<dist[ne.first][e.k]) {
				dist[ne.first][e.k]=e.d+ne.second;
				pq.push({ne.first, e.d+ne.second, e.k});
			}
			if(e.k<K&&e.d<dist[ne.first][e.k+1]) {
				dist[ne.first][e.k+1]=e.d;
				pq.push({ne.first, e.d, e.k+1});
			}
		}
	}
